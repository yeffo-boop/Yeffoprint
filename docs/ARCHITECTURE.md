...
- **Classic WooCommerce cart/checkout, not the Cart/Checkout blocks (Phase 7):** `woocommerce_get_item_data` — the hook that surfaces Size/Material/Batch details on the cart and checkout review — is a classic-template-era mechanism. It is *not* guaranteed to reach the newer Cart/Checkout blocks without an additional Store API schema-extension integration (`ExtendSchema`), which wasn't built this phase. Rather than force a specific choice by rewriting the site's Cart/Checkout page content (risking a destructive, surprising mutation of whatever a real site already has configured), Phase 7 assumes classic shortcode-based pages — the guaranteed-compatible path with the least integration risk — and styles that markup (`assets/css/woocommerce.css`) with some generic Blocks-class selectors included as a light-touch hedge. **Known gap:** if the site's Cart/Checkout pages actually use the Blocks, the custom item data won't display until that Store API extension is added.
